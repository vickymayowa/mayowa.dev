...
import { breadcrumbSchema, reviewSchema } from "@/lib/seo/json-ld"

const testimonials = [
    {
        id: "susan-akinkurolere",
...
export const metadata: Metadata = createPageMetadata({
  title: "Testimonials",
  description:
    "Read testimonials from clients and colleagues about working with Favour Mayowa on full-stack web development projects.",
  path: "/testimonials",
  keywords: ["client testimonials", "software engineer reviews"],
})
            ...
            <div className="container px-6 mx-auto max-w-5xl mt-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {testimonials.map((t) => (
                        ...
                        >
                            <Quote size={24} className="text-primary/10 absolute top-4 right-4 pointer-events-none transform group-hover:scale-110 transition-transform" aria-hidden="true" />

                            <div className="flex gap-1 mb-4 relative z-10" aria-label={`${t.rating} out of 5 stars`}>
                                {[...Array(t.rating)].map((_, i) => (
                                    <Star key={i} size={14} className="fill-primary text-primary" aria-hidden="true" />
                                ))}
                            </div>

                            <blockquote className="text-sm text-foreground/80 leading-relaxed mb-6 flex-grow italic relative z-10">
                                &quot;{t.content}&quot;
                            </blockquote>

                            <footer className="mt-auto border-t border-border/50 pt-4 flex items-center gap-4 relative z-10">
                                <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-colors" aria-hidden="true">
                                    {t.name.charAt(0)}
                                </div>
                                <div className="flex flex-col">
                                    <cite className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors not-italic">
                                        {t.name}
                                    </cite>
                ...
                <div className="card-premium relative overflow-hidden text-center p-8 md:p-12 border border-primary/20">
                    ...
                    <div className="relative z-10 flex flex-col items-center gap-4">
                        <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center mb-2">
                            <Star size={24} className="text-primary fill-primary/30" aria-hidden="true" />
                        </div>
                        ...
                        <Link href="/contact" className="btn-premium mt-6 group">
                            ...
                            <MessageSquare size={16} className="ml-2 group-hover:animate-bounce" aria-hidden="true" />
                        </Link>
                    </div>
                </div>
            </div>
